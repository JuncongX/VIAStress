# nohup python -u train_bcsa_multi.py --y_dim 2 --n_bcsa 3 > BCSA_2.out &
# nohup python -u train_bcsa_multi.py --y_dim 3 --n_bcsa 2 > BCSA_3.out &
# nohup python -u train_bcsa_multi.py --dataset_name ubfc_phys --ubfc_phys_task 2 --y_dim 2 --n_bcsa 3 --k 7 > BCSA_2_UP2.out &
# nohup python -u train_bcsa_multi.py --dataset_name ubfc_phys --ubfc_phys_task 3 --y_dim 2 --n_bcsa 4 --k 7 > BCSA_2_UP3.out &
# nohup python -u train_bcsa_multi.py --dataset_name can_stress --y_dim 2 --n_bcsa 3 > BCSA_2_CS.out &
# nohup python -u train_bcsa_multi.py --dataset_name verbio --y_dim 2 --n_bcsa 3 --k 5 > BCSA_2_VB.out &
import numpy as np
import os
...
from BCSA.bcsa import DAFMPPSR, Loss
...
def train(args, model, data_loader, criterion, optimizer):
    ...
    for j, data in enumerate(data_loader):
        ...
        labels, ppg, eda, scl = Variable(labels).to(device), Variable(ppg).to(device), Variable(eda).to(device), Variable(scl).to(device)

        batch_size, _, _ = eda.shape

        y_pred, y_pred_bvp, y_pred_eda = model(ppg, scl)
        optimizer.zero_grad()
        loss = criterion(y_pred_bvp, y_pred_eda, labels)
        loss.backward()
        optimizer.step()
        ...
        if train_cm is None:
            train_cm = confusion_matrix(labels.cpu().numpy(), predictions.cpu().numpy(), labels=[i for i in range(args.y_dim)])
        else:
            train_cm += confusion_matrix(labels.cpu().numpy(), predictions.cpu().numpy(), labels=[i for i in range(args.y_dim)])
...
def valid(args, model, data_loader):
    ...
    for j, data in enumerate(data_loader):
        ...
        if valid_cm is None:
            valid_cm = confusion_matrix(labels.cpu().numpy(), predictions.cpu().numpy(), labels=[i for i in range(args.y_dim)])
        else:
            valid_cm += confusion_matrix(labels.cpu().numpy(), predictions.cpu().numpy(), labels=[i for i in range(args.y_dim)])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_bcsa', type=int, default=2)
    ...
    for fold in range(args.k):
        ...
        model = DAFMPPSR(y_dim=args.y_dim, x_dim=args.x_dim, heads=args.heads, n_bcsa=args.n_bcsa)
        ...
        if args.dataset_name == 'wesad':
            ...
            valid_dataset = CAN_STRESS_dataset(valid_p)
        elif args.dataset_name == 'verbio':
            valid_dataset = VerBIO_dataset(valid_p)
        ...
        for epoch in range(args.epoch):
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save(model.module.state_dict(), os.path.join(args.save_path,
                                                                   'BCSA_{0}_{1}_{2}{3}.pth'.format(
                                                                       args.dataset_name,
                                                                       args.y_dim,
                                                                       "" if (
                                                                                   args.dataset_name != "ubfc_phys" or args.y_dim == 3) else "_UP{0}_".format(
                                                                           args.ubfc_phys_task),
                                                                       fold + 1)))
