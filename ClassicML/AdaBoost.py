...
base_estimator = DecisionTreeClassifier(criterion='entropy', min_samples_split=20, random_state=SEED)
def AB():
    return Pipeline([
        ('std_scaler', StandardScaler()),
        ('AB', AdaBoostClassifier(base_estimator=base_estimator, n_estimators=100, random_state=SEED))
        # ('AB', AdaBoostClassifier(estimator=base_estimator, n_estimators=100, random_state=SEED))
    ])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--k', type=int, default=5, help="KFold", choices=[5, 6, 7])
    ...
    for fold in range(args.k):
        ...
        joblib.dump(ab, 'save/AdaBoost_{0}{1}_{2}_{3}.pkl'.format(
            args.dataset_name,
