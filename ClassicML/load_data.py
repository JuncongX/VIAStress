...
class DataLoder():
    def __init__(self, dataset_name, binary=True, task=None):
        ...
        if dataset_name == 'wesad':
            ...
            self.data = np.load('WESAD_CML_clip30s.npy', allow_pickle=True)
            self.k = 5
            ...
            self.data = np.load('UBFC_Phys_CLM_clip30s.npy', allow_pickle=True)
            self.task = task
            self.k = 7
            ...
            self.data = np.load('VerBIO_CLM_clip30s.npy', allow_pickle=True)
            # with open("CAN_STRESS_CLM_clip30s.pkl", "rb") as f:
            #     data = pickle.load(f)
            self.k = 5
    ...
    def load(self, phase, fold):
        ...
        if self.dataset_name == "wesad":
            ...
            if self.binary:
                ...
                labels = np.array([l - 1 for l in needed_data[:, 1]]).astype(np.uint8)
