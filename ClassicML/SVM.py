...
def RBFKernelSVC(gamma=0.1):
    return Pipeline([
        ...
        ('svc', SVC(kernel='rbf', probability=True, gamma=gamma))
    ])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--y_dim', type=int, default=3, choices=[2, 3])
    ...
    for fold in range(args.k):
        ...
        svc_gamma = RBFKernelSVC()
        svc_gamma.fit(features_train, labels_train)
        res = svc_gamma.predict(features_test)
        ...
        joblib.dump(svc_gamma, 'save/SVM_{0}{1}_{2}_{3}.pkl'.format(
            args.dataset_name,
