# nohup python -u train_mixup.py --y_dim 2 > Mixup_2.out &
# nohup python -u train_mixup.py --y_dim 3 > Mixup_3.out &
# nohup python -u train_mixup.py --dataset_name ubfc_phys --ubfc_phys_task 2 --y_dim 2 --k 7 > Mixup_2_UP2.out &
# nohup python -u train_mixup.py --dataset_name ubfc_phys --ubfc_phys_task 3 --y_dim 2 --k 7 > Mixup_2_UP3.out &
# nohup python -u train_mixup.py --dataset_name can_stress --y_dim 2 > Mixup_2_CS.out &
# nohup python -u train_mixup.py --dataset_name verbio --y_dim 2 --k 5 > Mixup_2_VB.out &
import numpy as np
import os
...
def mixup_data(eda, ppg, y, device, alpha=1.0):
    ...
    batch_size = eda.size()[0]

    index = torch.randperm(batch_size).to(device)

    mixed_eda = lam * eda + (1 - lam) * eda[index, :]
    mixed_ppg = lam * ppg + (1 - lam) * ppg[index, :]
    y_a, y_b = y, y[index]
    return mixed_eda, mixed_ppg, y_a, y_b, lam
...
def train(args, model, data_loader, criterion, optimizer):
    ...
    for j, data in enumerate(data_loader):
        ...
        eda_mix, ppg_mix, targets_a, targets_b, lam = mixup_data(eda, ppg, labels, device, args.alpha)

        pred_logits = model(ppg_mix, eda_mix)
        optimizer.zero_grad()
        loss = mixup_criterion(criterion, pred_logits, targets_a, targets_b, lam)
        loss.backward()
        optimizer.step()
