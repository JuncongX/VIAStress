...
data = np.load('data/WESAD/WESAD_clip30s_multi.npy', allow_pickle=True)


# data = np.load('WESAD_clip30s_multi.npy', allow_pickle=True)
...
class WESAD_dataset(Dataset):
    def __init__(self, person_list, binary=True):
        ...
        self.PPG = torch.Tensor([ppg.astype(np.float32) for ppg in needed_data[:, 2]])
        self.SCL = torch.Tensor([ppg.astype(np.float32) for ppg in needed_data[:, 3]])
        self.SCR = torch.Tensor([ppg.astype(np.float32) for ppg in needed_data[:, 4]])
