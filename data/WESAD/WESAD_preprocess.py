...
def cut_signal(dir_, ppg_signal, label, save_datas):
    print(dir_)
    print(label)
    # ppg_signal = signal.resample(ppg_signal, int(len(ppg_signal) / 64 * resample_rate))
    # ppg_signal = detrend(ppg_signal)
    ppg_signal = butter_bandpass_filter(ppg_signal, resample_rate, low, high)
    total_times = (len(ppg_signal) - clip_len) / step
...
def get_data(number, save_datas):
    ...
    baseline_index_bvp = [int(i / 700 * 64) for i in baseline_index_ecg]
    stress_index_bvp = [int(i / 700 * 64) for i in stress_index_ecg]
    amusement_index_bvp = [int(i / 700 * 64) for i in amusement_index_ecg]
    ...
    cut_signal(dir_, bvp_baseline, 0, save_datas)
    cut_signal(dir_, bvp_stress, 1, save_datas)
    cut_signal(dir_, bvp_amusement, 2, save_datas)
...
if __name__ == '__main__':
    ...
    np.save("WESAD_clip{0}_order{1}_low{2}_high{3}.npy".format(clip_len, order, low, high), save_datas)
