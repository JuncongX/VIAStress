...
csv_file = r"ppg_quality_scores_101C.csv"
...
if __name__ == '__main__':
    ...
    for (eda_data, scr_data, scl_data, bvp_data, peak_data, participant_ids) in stress_data:
        ...
        try:
            # q_s = nk.ppg_quality(bvp_data, sampling_rate=ppg_sample_rate, method='disimilarity')
            q_s = nk.ppg_quality(bvp_data, sampling_rate=ppg_sample_rate, method='templatematch')
    ...
    # 构造 DataFrame 保存
    rows = []
    for pid, scores in quality_scores.items():
        # 展开所有嵌套层级
        scores_flat = list(itertools.chain.from_iterable(scores))

        rows.append({
            "Participant_ID": pid,
            # "Num_Segments": len(scores_flat),
            "Mean_Quality": np.nanmean(scores_flat),
            # "Median_Quality": np.nanmedian(scores_flat),
            # "Scores": scores_flat
        })
    ...
    print(f"✅ PPG quality scores saved to: {output_file}")
