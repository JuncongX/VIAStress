...
def find_peak(ppg_signal):
    ...
    peak_ = signal.find_peaks(ppg_signal, distance=10)[0]
...
def get_data(task, dir_, save_datas):
    ...
    ppg_signal = signal.resample(ppg_signal, len(bvp_signal))

    ppg_signal = ppg_signal[drop_num:]
    bvp_signal = bvp_signal[drop_num:]
    bvp_signal = z_score(bvp_signal)
    # bvp_signal = butter_bandpass_filter(bvp_signal, ppg_fps, cutoff_low, cutoff_high)
    ...
    for times in range(math.floor(total_times)):
        ...
        datas = [dir_, task, level, label, ppg_signal_, bvp_signal_]
...
if __name__ == '__main__':
    ...
    np.save("ubfc_phys_clip{0}.npy".format(clip_len), save_datas)
