...
from VIAStress.Ablation.wo_GaMe.wo_GaMe import Model, BVPEncoder, BVPDecoder, EDAEncoder, EDADecoder, \
        ...
        for epoch in range(args.epoch):
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save(model.state_dict(), os.path.join(args.save_path,
                                                            'VAStress_wo_game_{0}_{1}_{2}{3}.pth'.format(
                                                                args.dataset_name,
                                                                args.y_dim,
                                                                model_name_tool(args),
                                                                fold + 1)))
                ...
                torch.save({"bvp_proj": bvp_proj.state_dict(), "eda_proj": eda_proj.state_dict(),
                           ...
                           os.path.join(args.save_path,
                                        'VAStress_r_wcn_pt_{0}_{1}_{2}{3}.pth'.format(
                                            args.dataset_name,
                                            args.y_dim,
                                            model_name_tool(args),
                                            fold + 1)))
