...
from VIAStress.Ablation.only_EDA.only_EDA import Model, EDAEncoder, EDADecoder
...
def train(args, model, data_loader, criterion, optimizer):
    ...
    for j, data in enumerate(data_loader):
        ...
        pred_logits, recon_eda, mu_eda, logvar_eda = model(eda)
...
def valid(args, model, data_loader):
    ...
    for j, data in enumerate(data_loader):
        ...
        pred_logits, recon_bvp, mu_bvp, logvar_bvp = model(eda)
...
if __name__ == '__main__':
    ...
    for fold in range(args.k):
        ...
        optimizer = torch.optim.Adam([
            ...
            {'params': model.eda_encoder_vae.parameters(), 'lr': 1e-4},  # 设置较低的学习率
        ...
        for epoch in range(args.epoch):
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save(model.state_dict(), os.path.join(args.save_path,
                                                            'VAStress_only_eda_{0}_{1}_{2}{3}.pth'.format(
                                                                args.dataset_name,
                                                                args.y_dim,
                                                                model_name_tool(args),
                                                                fold + 1)))
