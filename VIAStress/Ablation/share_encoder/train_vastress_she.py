...
from VIAStress.Ablation.share_encoder.SHE import Model, BVPEncoder, BVPDecoder, EDAEncoder, EDADecoder
...
if __name__ == '__main__':
    ...
    for fold in range(args.k):
        ...
        optimizer = torch.optim.Adam([
            {'params': model.bvp_encoder_fc.parameters(), 'lr': 1e-3},  # 设置较高的学习率
            {'params': model.eda_encoder_fc.parameters(), 'lr': 1e-3},
        ...
        for epoch in range(args.epoch):
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save(model.state_dict(), os.path.join(args.save_path,
                                                            'VAStress_she_{0}_{1}_{2}{3}.pth'.format(
                                                                args.dataset_name,
                                                                args.y_dim,
                                                                model_name_tool(args),
                                                                fold + 1)))
                ...
                torch.save({"bvp_proj": bvp_proj.state_dict(), "eda_proj": eda_proj.state_dict(),
                           ...
                           os.path.join(args.save_path,
                                        'VAStress_she_pt_{0}_{1}_{2}{3}.pth'.format(
                                            args.dataset_name,
                                            args.y_dim,
                                            model_name_tool(args),
                                            fold + 1)))
