...
from VIAStress.Ablation.only_PPG.only_PPG import Model, BVPEncoder, BVPDecoder
...
class Loss(nn.Module):
    ...
    def forward(self,
        ...
        # bvp_l1 = self.l1_loss(recon_bvp, ppg)
        ...
        return ce_loss + bvp_peak_ce + bvp_kl
...
def train(args, model, data_loader, criterion, optimizer):
    ...
    for j, data in enumerate(data_loader):
        ...
        pred_logits, recon_bvp, mu_bvp, logvar_bvp = model(ppg)
        ...
        for epoch in range(args.epoch):
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save(model.state_dict(), os.path.join(args.save_path,
                                                            'VAStress_only_ppg_{0}_{1}_{2}{3}.pth'.format(
                                                                args.dataset_name,
                                                                args.y_dim,
                                                                model_name_tool(args),
                                                                fold + 1)))
