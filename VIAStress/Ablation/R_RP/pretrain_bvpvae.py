# nohup python -u pretrain_bvpvae.py > PREVAEAD_PPG_w.out &
# nohup python -u pretrain_bvpvae.py --dataset_name ubfc_phys --k 7 --ubfc_phys_task 2 > PREVAEAD_PPG_u2.out &
# nohup python -u pretrain_bvpvae.py --dataset_name ubfc_phys --k 7 --ubfc_phys_task 3 > PREVAEAD_PPG_u3.out &
# nohup python -u pretrain_bvpvae.py --dataset_name verbio --k 5 > PREVAEAD_PPG_vb.out &
...
from VIAStress.Ablation.R_RP.R_RP import BVPEncoder, BVPDecoder
...
def train(args, encoder, decoder, data_loader, criterion, optimizer):
    ...
    for j, data in enumerate(data_loader):
        labels, ppg, scr, scl, eda, _ = data
        # eda = torch.cat((scr, scl), dim=1)
        # peak = Variable(peak).to(device)

        min_vals = ppg.min(dim=2, keepdim=True).values  # shape: (3, 1, 1)
        max_vals = ppg.max(dim=2, keepdim=True).values  # shape: (3, 1, 1)
        denom = max_vals - min_vals
        denom[denom == 0] = 1
        normalized_ppg = 2 * (ppg - min_vals) / denom - 1

        signal = Variable(normalized_ppg).to(device)

        batch_size, _, _ = ppg.shape
...
if __name__ == '__main__':
    ...
    for fold in range(args.k):
        ...
        for epoch in range(args.epoch):
            ...
            if valid_person >= best_pc and epoch > 50:
                best_ever = valid_loss
                best_pc = valid_person
                torch.save(encoder.state_dict(),
                           os.path.join(args.save_path, 'Pre_VAEAD_encoder_PPG_{0}_{1}_{2}{3}.pth'.format(
                               args.dataset_name,
                               args.y_dim,
                               model_name_tool(args),
                               fold + 1)))
                torch.save(decoder.state_dict(),
                           os.path.join(args.save_path, 'Pre_VAEAD_decoder_PPG_{0}_{1}_{2}{3}.pth'.format(
                               args.dataset_name,
                               args.y_dim,
                               model_name_tool(args),
                               fold + 1)))
