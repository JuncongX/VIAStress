...
from VIAStress.Ablation.R_RP.R_RP import Model, BVPEncoder, BVPDecoder, EDAEncoder, EDADecoder, ProjectHead, \
    EncoderTrans
...
class Loss(nn.Module):
    def __init__(self, args):
        ...
        self.bvp_L1 = nn.MSELoss(reduction="none")
    ...
    def forward(self,
        ...
        bvp_expanded = ppg.unsqueeze(0).expand_as(recon_bvp)  # (5, B, 1, 1920)
        bvp_loss = self.bvp_L1(recon_bvp, bvp_expanded)
        bvp_l1 = bvp_loss.mean()  # average over all dims
...
def train(args, model, bvp_proj, eda_proj, mlp_b2e, mlp_e2b, data_loader, criterion, criterion_contrast, optimizer):
    ...
    for j, data in enumerate(data_loader):
        ...
        min_vals_ppg = ppg.min(dim=2, keepdim=True).values  # shape: (3, 1, 1)
        max_vals_ppg = ppg.max(dim=2, keepdim=True).values  # shape: (3, 1, 1)
        denom_ppg = max_vals_ppg - min_vals_ppg
        denom_ppg[denom_ppg == 0] = 1
        normalized_ppg = 2 * (ppg - min_vals_ppg) / denom_ppg - 1

        min_vals_eda = eda.min(dim=2, keepdim=True).values  # shape: (3, 1, 1)
        max_vals_eda = eda.max(dim=2, keepdim=True).values  # shape: (3, 1, 1)
        denom_eda = max_vals_eda - min_vals_eda
        denom_eda[denom_eda == 0] = 1
        normalized_eda = 2 * (eda - min_vals_eda) / denom_eda - 1

        loss = criterion(pred_logits, labels,
                         recon_bvp, normalized_ppg,
                         recon_eda, normalized_eda,
                         mu_bvp, logvar_bvp,
                         mu_eda, logvar_eda)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prerp_save_path', type=str, default="./checkpoints_rpspre")
    ...
    for fold in range(args.k):
        ...
        if args.pre_train:
            ...
            bvp_encoder.load_state_dict(torch.load(os.path.join(args.pre_save_path,
                                                                'Pre_VAEAD_encoder_PPG_{0}_{1}_{2}{3}.pth'.format(
                                                                    args.dataset_name,
                                                                    2,
            ...
            bvp_decoder.load_state_dict(torch.load(os.path.join(args.pre_save_path,
                                                                'Pre_VAEAD_decoder_PPG_{0}_{1}_{2}{3}.pth'.format(
                                                                    args.dataset_name,
                                                                    2,
                                                                    model_name_tool(args),
                                                                    fold + 1))))
        ...
        for epoch in range(args.epoch):
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save(model.state_dict(), os.path.join(args.save_path,
                                                            'VAStress_r_rp_{0}_{1}_{2}{3}.pth'.format(
                                                                args.dataset_name,
                                                                args.y_dim,
                                                                model_name_tool(args),
                                                                fold + 1)))
                ...
                torch.save({"bvp_proj": bvp_proj.state_dict(), "eda_proj": eda_proj.state_dict(),
                           ...
                           os.path.join(args.save_path,
                                        'VAStress_r_rp_pt_{0}_{1}_{2}{3}.pth'.format(
                                            args.dataset_name,
                                            args.y_dim,
                                            model_name_tool(args),
                                            fold + 1)))
