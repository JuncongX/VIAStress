...
from VIAStress.Ablation.R_AE.R_AE import Model, BVPEncoder, BVPDecoder, EDAEncoder, EDADecoder, ProjectHead, \
    EncoderTrans
...
class Loss(nn.Module):
    def __init__(self):
        ...
        self.eda_L1 = nn.MSELoss(reduction="mean")
    ...
    def forward(self,
        ...
        bvp_ce = F.binary_cross_entropy(recon_bvp, peak, reduction="mean")
        eda_l1 = self.eda_L1(recon_eda, eda)

        return ce_loss + bvp_ce + eda_l1
...
def train(args, model, bvp_proj, eda_proj, mlp_b2e, mlp_e2b, data_loader, criterion, criterion_contrast, optimizer):
    ...
    for j, data in enumerate(data_loader):
        ...
        pred_logits, bvp_z, eda_z, recon_bvp, recon_eda = model(ppg, eda)
        ...
        for epoch in range(args.epoch):
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save(model.state_dict(), os.path.join(args.save_path,
                                                            'VAStress_r_ae_{0}_{1}_{2}{3}.pth'.format(
                                                                args.dataset_name,
                                                                args.y_dim,
                                                                model_name_tool(args),
                                                                fold + 1)))
                torch.save({"bvp_proj": bvp_proj.state_dict(), "eda_proj": eda_proj.state_dict(),
                           ...
                           os.path.join(args.save_path,
                                        'VAStress_r_ae_pt_{0}_{1}_{2}{3}.pth'.format(
                                            args.dataset_name,
                                            args.y_dim,
                                            model_name_tool(args),
                                            fold + 1)))
