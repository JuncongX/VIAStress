...
from VIAStress.param_tool import out_dim_tool, exp_param_tool
...
class Loss(nn.Module):
    def __init__(self):
        ...
        self.eda_L2 = nn.MSELoss(reduction="none")
    ...
    def forward(self,
        ...
        eda_loss = self.eda_L2(recon_eda, eda_expanded)
        eda_l2 = eda_loss.mean()
        ...
        # Total Loss
        total_loss = ce_loss + bvp_peak_ce + 0.5 * bvp_kl + eda_l2 + 0.1 * eda_kl
        return total_loss
...
def train(args, model, bvp_proj, eda_proj, mlp_b2e, mlp_e2b, data_loader, criterion, criterion_contrast, optimizer):
    ...
    for j, data in enumerate(data_loader):
        ...
        # loss_e += (1 + F.cosine_similarity(bvp_z[:, :dim_], bvp_z[:, dim_:])).mean()
        # loss_e += (1 + F.cosine_similarity(eda_z[:, :dim_], eda_z[:, dim_:])).mean()
        loss_e = loss_e / 2
        loss += loss_e * 0.1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pre_save_path_l2', type=str, default="./checkpoints_edal2")
    ...
    for fold in range(args.k):
        ...
        if args.pre_train:
            ...
            eda_encoder.load_state_dict(torch.load(os.path.join(args.pre_save_path_l2,
            ...
            eda_decoder.load_state_dict(torch.load(os.path.join(args.pre_save_path_l2,
        ...
        for epoch in range(args.epoch):
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save(model.state_dict(), os.path.join(args.save_path,
                                                            'VAStress_r_l2_{0}_{1}_{2}{3}.pth'.format(
                                                                args.dataset_name,
                                                                args.y_dim,
                                                                model_name_tool(args),
                                                                fold + 1)))
                ...
                torch.save({"bvp_proj": bvp_proj.state_dict(), "eda_proj": eda_proj.state_dict(),
                           ...
                           os.path.join(args.save_path,
                                        'VAStress_r_l2_pt_{0}_{1}_{2}{3}.pth'.format(
                                            args.dataset_name,
                                            args.y_dim,
                                            model_name_tool(args),
                                            fold + 1)))
