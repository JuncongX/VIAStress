...
from VIAStress.bin.R_MetaAge.R_MetaAge import Model, BVPEncoder, BVPDecoder, EDAEncoder, EDADecoder, \
    ProjectHead, EncoderTrans
    ...
    for fold in range(args.k):
        ...
        if args.pre_train:
            ...
            model = Model(y_dim=args.y_dim, r_dim=args.r_dim, x_dim=args.x_dim, h_dim=args.h_dim,
                          ...
                          mlp_dim=[(args.h_dim + args.r_dim), 2 * args.h_dim, args.h_dim])
        else:
            model = Model(y_dim=args.y_dim, r_dim=args.r_dim, x_dim=args.x_dim, h_dim=args.h_dim,
                          mlp_dim=[(args.h_dim + args.r_dim), args.h_dim, args.h_dim])
        ...
        for epoch in range(args.epoch):
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save(model.state_dict(), os.path.join(args.save_path,
                                                            'VAStress_r_metaage_{0}_{1}_{2}{3}.pth'.format(
                                                                args.dataset_name,
                                                                args.y_dim,
                                                                model_name_tool(args),
                                                                fold + 1)))
                ...
                torch.save({"bvp_proj": bvp_proj.state_dict(), "eda_proj": eda_proj.state_dict(),
                           ...
                           os.path.join(args.save_path,
                                        'VAStress_r_metaage_pt_{0}_{1}_{2}{3}.pth'.format(
                                            args.dataset_name,
                                            args.y_dim,
                                            model_name_tool(args),
                                            fold + 1)))
