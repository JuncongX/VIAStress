# python draw_tsne_me_db_linear.py --dataset_name wesad --k 5 --od 64 --exp 1 --y_dim 2
# python draw_tsne_me_db_linear.py --dataset_name wesad --k 5 --od 64 --exp 0.5 --y_dim 3
# python draw_tsne_me_db_linear.py --dataset_name ubfc_phys --k 7 --od 128 --y_dim 2 --exp 1.5 --ubfc_phys_task 2
# python draw_tsne_me_db_linear.py --dataset_name ubfc_phys --k 7 --od 128 --y_dim 2 --exp 0.5 --ubfc_phys_task 3
...
for fold in range(args.k):
    ...
    # 为每个 subject 设置透明度（均匀分布在一定范围，例如 0.3~0.9）
    subject_alpha_map = {
        subj: 0.2 + 0.8 * (i / max(1, num_subjects - 1))
    ...
    for subj in unique_subjects:
        ...
        # 绘制该 subject 的决策边界（透明背景）
        Z = clf.predict(np.c_[xx.ravel(), yy.ravel()])
        Z = Z.reshape(xx.shape)
        cmap_light = ListedColormap([colors[i] for i in range(len(np.unique(labels)))])
        plt.contourf(xx, yy, Z, cmap=cmap_light, alpha=0.05)  # 所有 subject 边界叠加

        # 绘制该 subject 的样本点（颜色为 class，透明度为 subject）
        for cls in np.unique(y):
            idx = (y == cls)
            plt.scatter(X[idx, 0], X[idx, 1],
                        color=colors[cls],
                        marker=marker_dict.get(cls, 'x'),
                        alpha=subject_alpha_map[subj],
                        edgecolors='none')

    # 图像修饰
    plt.xticks([])
    plt.yticks([])
    # 图例（只显示 class，不重复 subject）
    handles = [
        Line2D([0], [0],
               marker=marker_dict.get(cls, 'x'),
               color='w',
               ...
               # markeredgecolor='k',
               markersize=10,
               label=class_name_dict.get(cls, str(cls)))
    ...
    plt.savefig(f"tsne_plots/tsne_fold{fold + 1}_all_subjects_points_and_boundaries_alpha.png")
    plt.close()