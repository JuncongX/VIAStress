...
def train(args, model, bvp_proj, eda_proj, mlp_b2e, mlp_e2b, data_loader, criterion, criterion_contrast, optimizer):
    ...
    for j, data in enumerate(data_loader):
        ...
        # # Cycle-Consistency
        # bvp_emd_t_c = mlp_e2b(eda_emd_t)
        # eda_emd_t_c = mlp_b2e(bvp_emd_t)
        # e2bc_loss = torch.mean(
        #     torch.norm(bvp_emd_t_c - bvp_z[:, :dim_] / torch.norm(bvp_z[:, :dim_], dim=1, keepdim=True), dim=1))
        # b2ec_loss = torch.mean(
        #     torch.norm(eda_emd_t_c - eda_z[:, :dim_] / torch.norm(eda_z[:, :dim_], dim=1, keepdim=True), dim=1))
        # loss += (e2bc_loss + b2ec_loss) / 2 * 0.1
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save({"bvp_proj": bvp_proj.state_dict(), "eda_proj": eda_proj.state_dict(),
                           ...
                           os.path.join(args.save_path,
                                        'VAEAD_me_pt_{}_{}_{}_{}_{}{}.pth'.format(
                                            exp_param_tool(args),
                                            out_dim_tool(args),
                                            args.dataset_name,
                                            args.y_dim,
