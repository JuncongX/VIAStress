...
from VIAStress.model import ProjectHead, EncoderTrans
...
def train(args, model, bvp_proj, eda_proj, mlp_b2e, mlp_e2b, data_loader, criterion, criterion_contrast, optimizer):
    ...
    for j, data in enumerate(data_loader):
        ...
        pred_logits, bvp_z, eda_z = model(ppg, eda)
        ...
        # loss_e += - F.mse_loss(bvp_z[:, :dim_], bvp_z[:, dim_:])
        loss_e += torch.exp(-exp_param_tool(args) * F.mse_loss(eda_z[:, :dim_], eda_z[:, dim_:]))
        # loss_e += - F.mse_loss(eda_z[:, :dim_], eda_z[:, dim_:])

        loss_e = loss_e / 2
        loss += loss_e * 0.1

        # Cross-modal Translation
        bvp_emd_t = mlp_e2b(eda_z)
        eda_emd_t = mlp_b2e(bvp_z)
        e2b_loss = torch.mean(
            torch.norm(bvp_emd_t - bvp_z / torch.norm(bvp_z, dim=1, keepdim=True), dim=1))
        b2e_loss = torch.mean(
            torch.norm(eda_emd_t - eda_z / torch.norm(eda_z, dim=1, keepdim=True), dim=1))
...
if __name__ == '__main__':
    ...
    for fold in range(args.k):
        ...
        mlp_b2e = EncoderTrans(256, 256, 256).to(device)
        mlp_e2b = EncoderTrans(256, 256, 256).to(device)
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save({"bvp_proj": bvp_proj.state_dict(), "eda_proj": eda_proj.state_dict(),
                           ...
                           os.path.join(args.save_path,
                                        'SimMMDG_pt_{0}_{1}_{2}{3}.pth'.format(
                                            args.dataset_name,
                                            args.y_dim,
                                            model_name_tool(args),
                                            fold + 1)))
