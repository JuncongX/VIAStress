# nohup python -u train_vaead_pre.py --y_dim 2 > VAEAD_pre_2.out &
# nohup python -u train_vaead_pre.py --y_dim 3 > VAEAD_pre_3.out &
# nohup python -u train_vaead_pre.py --dataset_name ubfc_phys --ubfc_phys_task 2 --y_dim 2 --k 7 > VAEAD_pre_2_UP2.out &
# nohup python -u train_vaead_pre.py --dataset_name ubfc_phys --ubfc_phys_task 3 --y_dim 2 --k 7 > VAEAD_pre_2_UP3.out &
# nohup python -u train_vaead_pre.py --dataset_name ubfc_phys --y_dim 3 --k 7 > VAEAD_pre_3_UP.out &
import numpy as np
import os
...
if __name__ == '__main__':
    ...
    for fold in range(args.k):
        ...
        if args.pre_train:
            pre_x_dim = 128
            bvp_encoder = BVPEncoder(r_dim=args.r_dim, x_dim=pre_x_dim)
            ...
            eda_encoder = EDAEncoder(r_dim=args.r_dim, x_dim=pre_x_dim)
        ...
        optimizer = torch.optim.Adam([
            {'params': model.feature_cnn.parameters(), 'lr': 1e-3},
            # {'params': model.feature_fc.parameters(), 'lr': 1e-3},
            {'params': model.bvp_encoder_vae.parameters(), 'lr': 1e-4},
        ...
        for epoch in range(args.epoch):
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save(model.state_dict(), os.path.join(args.save_path,
                                                                   'VAEAD_pre_{0}_{1}_{2}{3}.pth'.format(
                                                                       args.dataset_name,
                                                                       args.y_dim,
                                                                       model_name_tool(args),
                                                                       fold + 1)))
