...
class CAN_STRESS_dataset(Dataset):
    def __init__(self, person_list):

        needed_data = None
        for person in person_list:
            selected_data = data[(data[:, 0] == person)]
            if needed_data is None:
                needed_data = selected_data
            else:
                needed_data = np.concatenate((needed_data, selected_data))

        self.labels = torch.from_numpy(np.array([BINARY_LABEL_DICT[l] for l in needed_data[:, 1]]).astype(np.uint8))
...
if __name__ == '__main__':
    ...
    all_persons = []
    dataset = CAN_STRESS_dataset(all_persons)
