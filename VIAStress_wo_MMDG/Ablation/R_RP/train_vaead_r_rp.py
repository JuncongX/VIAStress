...
from VIAStress_wo_MMDG.Ablation.R_RP.R_RP import Model, BVPEncoder, BVPDecoder, EDAEncoder, EDADecoder
...
class Loss(nn.Module):
    def __init__(self):
        ...
        self.ppg_L2 = nn.MSELoss(reduction="mean")
    ...
    def forward(self,
        ...
        bvp_l2 = self.ppg_L2(recon_bvp, ppg)
    ...
    for fold in range(args.k):
        ...
        if args.pre_train:
            ...
            bvp_encoder.load_state_dict(torch.load(os.path.join(args.pre_save_path,
                                                                'Pre_VAEAD_encoder_RP_{0}_{1}_{2}{3}.pth'.format(
                                                                    args.dataset_name,
                                                                    2,
            ...
            bvp_decoder.load_state_dict(torch.load(os.path.join(args.pre_save_path,
                                                                'Pre_VAEAD_decoder_RP_{0}_{1}_{2}{3}.pth'.format(
                                                                    args.dataset_name,
                                                                    2,
                                                                    model_name_tool(args),
                                                                    fold + 1))))
        ...
        for epoch in range(args.epoch):
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save(model.state_dict(), os.path.join(args.save_path,
                                                            'VAEAD_r_rp_{0}_{1}_{2}{3}.pth'.format(
                                                                args.dataset_name,
                                                                args.y_dim,
                                                                model_name_tool(args),
                                                                fold + 1)))
