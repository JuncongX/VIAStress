# nohup python -u pretrain_rpvae.py > PREVAEAD_RP_w.out &
# nohup python -u pretrain_rpvae.py --dataset_name ubfc_phys --k 7 --ubfc_phys_task 2 > PREVAEAD_RP_u2.out &
# nohup python -u pretrain_rpvae.py --dataset_name ubfc_phys --k 7 --ubfc_phys_task 3 > PREVAEAD_RP_u3.out &
...
from VIAStress_wo_MMDG.model import BVPEncoder, BVPDecoder
        ...
        for epoch in range(args.epoch):
            ...
            if valid_loss <= best_ever:
                ...
                torch.save(encoder.state_dict(),
                           os.path.join(args.save_path, 'Pre_VAEAD_encoder_RP_{0}_{1}_{2}{3}.pth'.format(
                               args.dataset_name,
                               args.y_dim,
                               model_name_tool(args),
                               fold + 1)))
                torch.save(decoder.state_dict(),
                           os.path.join(args.save_path, 'Pre_VAEAD_decoder_RP_{0}_{1}_{2}{3}.pth'.format(
                               args.dataset_name,
                               args.y_dim,
                               model_name_tool(args),
                               fold + 1)))
