...
from VIAStress_wo_MMDG.Ablation.R_AE.R_AE import Model, BVPEncoder, BVPDecoder, EDAEncoder, EDADecoder
...
class Loss(nn.Module):
    ...
    def forward(self,
        ...
        bvp_peak_ce = F.binary_cross_entropy(recon_bvp, ppg, reduction="mean")
        eda_l2 = self.eda_L2(recon_eda, eda)

        return ce_loss + bvp_peak_ce + eda_l2
...
def train(args, model, data_loader, criterion, optimizer):
    ...
    for j, data in enumerate(data_loader):
        ...
        pred_logits, recon_bvp, recon_eda = model(ppg, eda)
        optimizer.zero_grad()
    ...
    for fold in range(args.k):
        ...
        if args.pre_train:
            ...
            bvp_encoder.load_state_dict(torch.load(os.path.join(args.pre_save_path,
                                                                'Pre_AEAD_encoder_PPG_{0}_{1}_{2}{3}.pth'.format(
                                                                    args.dataset_name,
                                                                    2,
            ...
            bvp_decoder.load_state_dict(torch.load(os.path.join(args.pre_save_path,
                                                                'Pre_AEAD_decoder_PPG_{0}_{1}_{2}{3}.pth'.format(
                                                                    args.dataset_name,
                                                                    2,
                                                                    model_name_tool(args),
                                                                    fold + 1))))
            ...
            eda_encoder.load_state_dict(torch.load(os.path.join(args.pre_save_path,
                                                                'Pre_AEAD_encoder_EDA_{0}_{1}_{2}{3}.pth'.format(
                                                                    args.dataset_name,
                                                                    2,
                                                                    model_name_tool(args),
                                                                    fold + 1))))
            ...
            eda_decoder.load_state_dict(torch.load(os.path.join(args.pre_save_path,
                                                                'Pre_AEAD_decoder_EDA_{0}_{1}_{2}{3}.pth'.format(
                                                                    args.dataset_name,
                                                                    2,
                                                                    model_name_tool(args),
                                                                    fold + 1))))
            ...
            model = Model(y_dim=args.y_dim, r_dim=args.r_dim, x_dim=args.x_dim, h_dim=args.h_dim,
                          bvp_encoder_ae=bvp_encoder, bvp_decoder_ae=bvp_decoder,
                          eda_encoder_ae=eda_encoder, eda_decoder_ae=eda_decoder)
        else:
            model = Model(y_dim=args.y_dim, r_dim=args.r_dim, x_dim=args.x_dim, h_dim=args.h_dim)
        ...
        for epoch in range(args.epoch):
            ...
            if epoch_valid_f1 >= bestf1_ever:
                ...
                torch.save(model.state_dict(), os.path.join(args.save_path,
                                                            'VAEAD_r_ae_{0}_{1}_{2}{3}.pth'.format(
                                                                args.dataset_name,
                                                                args.y_dim,
                                                                model_name_tool(args),
                                                                fold + 1)))
