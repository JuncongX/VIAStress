...
class MemoryGatedEncoder(nn.Module):
    ...
    def forward(self, q):
        ...
        gate = self.gate_fc(q)  # [B, r_dim]
        out = gate * q + (1 - gate) * memory_read  # gated fusion
...
class ADClassifier(nn.Module):
    def __init__(self, z_dim, r_dim, h_dim, y_dim):
        ...
        #     # nn.BatchNorm1d(self.r_dim),
        #     # nn.ReLU(),
        #     nn.Linear(self.r_dim, self.r_dim),
        #     # nn.Linear(2 * self.r_dim, self.r_dim),
        #     nn.ReLU(),
        #     nn.Linear(self.r_dim, self.r_dim),
        #     # nn.Linear(2 * self.r_dim, self.r_dim),
        #     nn.ReLU(),
        ...
        self.r_enc_1 = MemoryGatedEncoder(self.r_dim)
