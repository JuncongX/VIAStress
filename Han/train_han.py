# nohup python -u train_han.py --y_dim 2 > Han_2.out &
# nohup python -u train_han.py --y_dim 3 > Han_3.out &
# nohup python -u train_han.py --dataset_name ubfc_phys --ubfc_phys_task 2 --y_dim 2 --k 7 > Han_2_UP2.out &
# nohup python -u train_han.py --dataset_name ubfc_phys --ubfc_phys_task 3 --y_dim 2 --k 7 > Han_2_UP3.out &
# nohup python -u train_han.py --dataset_name can_stress --y_dim 2 > Han_2_CS.out &
# nohup python -u train_han.py --dataset_name verbio --y_dim 2 --k 5 > Han_2_VB.out &
import numpy as np
import os
...
from Han.data_augmentation import jitter, time_warp, magnitude_warp, scaling
...
def consistency_loss(pred_logits, augmented_logits_list):
    """KL散度一致性损失: 对每个增强输出与原输出求KL散度再平均"""
    ...
    for aug_logits in augmented_logits_list:
        ...
        total_kl += kl.mean()
    return total_kl / len(augmented_logits_list)


def Han_criterion(criterion, pred_logits, labels, epoch, M, E_warmup=50, augmented_logits_list=None):
    """总损失 = 监督交叉熵 + α * 一致性正则化"""
    ce_loss = criterion(pred_logits, labels)
    if augmented_logits_list is not None:
        alpha = np.exp((np.min(epoch / E_warmup, 1)) ** 2) / M
        cons_loss = consistency_loss(pred_logits, augmented_logits_list)
        total_loss = ce_loss + alpha * cons_loss
    else:
        total_loss = ce_loss
    return total_loss
...
def train(args, model, data_loader, criterion, optimizer, epoch):
    ...
    for j, data in enumerate(data_loader):
        ...
        # ppg_np = ppg.detach().cpu().numpy()
        # eda_np = eda.detach().cpu().numpy()
        #
        # ppg_jitter = torch.from_numpy(jitter(ppg_np))
        # eda_jitter = torch.from_numpy(jitter(eda_np))
        # augmented_logits_list.append(model(ppg_jitter, eda_jitter))
        #
        # ppg_scaling = torch.from_numpy(scaling(ppg_np))
        # eda_scaling = torch.from_numpy(scaling(eda_np))
        # augmented_logits_list.append(model(ppg_scaling, eda_scaling))
        #
        # ppg_time_warp = torch.from_numpy(time_warp(ppg_np))
        # eda_time_warp = torch.from_numpy(time_warp(eda_np))
        # augmented_logits_list.append(model(ppg_time_warp, eda_time_warp))
        #
        # ppg_magnitude_warp = torch.from_numpy(magnitude_warp(ppg_np))
        # eda_magnitude_warp = torch.from_numpy(magnitude_warp(eda_np))
        # augmented_logits_list.append(model(ppg_magnitude_warp, eda_magnitude_warp))

        optimizer.zero_grad()
        # loss = Han_criterion(criterion, pred_logits, labels, epoch, 5, augmented_logits_list=augmented_logits_list)
...
if __name__ == '__main__':
    ...
    for fold in range(args.k):
        ...
        model = model_conv1d(config, num_classes=args.y_dim)
        ...
        for epoch in range(args.epoch):
            ...
            for i, train_p_one in enumerate(train_p):
                ...
                train_loss, train_correct, train_cm = train(args, model, train_loader, criterion, optimizer, epoch)
