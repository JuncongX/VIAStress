...
if __name__ == "__main__":
    ...
    parser.add_argument('--cd', type=str, default='w2u', choices=['w2u', 'u2w', 'w2v', 'u2v', 'v2w', 'v2u'])
    ...
    if args.cd[-1] == 'w':
        test_dataset = WESAD_dataset(all_subject, binary=True if args.class_num == 2 else False)
    elif args.cd[-1] == 'u':
        if args.class_num == 2:
    ...
    for fold in range(args.k):
        ...
        algorithm_dict = torch.load(os.path.join(args.save_path,
                                                 f"{args.algorithm}_{model_name(args)}_{args.class_num}_{model_name_tool(args)}{fold}"))
        model.load_state_dict(algorithm_dict)
        model.to(device)

        acc, f1, recall, precision = f1_accuracy(model, test_loader, args.class_num, device)
        print(
