...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name', type=str, default='wesad', choices=['wesad', 'ubfc_phys', 'verbio', 'can_stress'])
    ...
    if args.dataset_name == 'wesad':
        ...
        from data.CAN_STRESS.Uniform_distribution_person_multi_0_4 import test_subject
    elif args.dataset_name == 'verbio':
    ...
    for fold in range(args.k):
        ...
        algorithm_dict = torch.load(os.path.join(args.save_path,
        ...
        for i, test_p_one in enumerate(test_p):
            if args.dataset_name == 'wesad':
                test_dataset = WESAD_dataset([test_p_one], binary=True if args.class_num == 2 else False)
            elif args.dataset_name == 'ubfc_phys':
                if args.class_num == 2:
            ...
            cm = f1_accuracy(model, test_loader, args.class_num, device)
            if confusion_mats is None:
                confusion_mats = cm
            else:
                confusion_mats += cm
        acc, f1, recall, precision = f1_score_from_confusion_matrix(confusion_mats, args.class_num)
        print(
