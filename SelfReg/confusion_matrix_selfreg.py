# python confusion_matrix_selfreg.py --dataset_name wesad --k 5 --y_dim 2
# python confusion_matrix_selfreg.py --dataset_name ubfc_phys --k 7 --y_dim 2 --ubfc_phys_task 2
...
def f1_accuracy(algorithm, loader, class_num, device):
    cm = None
...
if __name__ == "__main__":
    ...
    for fold in range(args.k):
        ...
        state_dict = torch.load(os.path.join(args.save_path,
        ...
        for k, v in state_dict.items():
            # 只处理目标模型中包含的键
            if k in algorithm.state_dict():
                new_state_dict[k] = v
            else:
                print(f"Skipping key {k} since it's not in the target model")
        algorithm.load_state_dict(new_state_dict)
        algorithm.to(device)

        acc, f1, recall, precision = f1_accuracy(algorithm, test_loader, args.y_dim, device)
        print(
